...
// const rateSchema = Joi.object({ rating: Joi.number().integer().min(1).max(5).required() })
...
// 	useEffect(() => { load(); if (user) loadMyRatings() }, [user])
...
// 			const map = {}
// 			data.forEach(r => { map[r.storeId] = r.rating })
// 			setMyRatings(map)
// 		} catch {}
// 	}
...
// 		try {
// 			if (myRatings[storeId]) await axios.post('/stores/update-rating', { storeId, rating: value })
// 			else await axios.post('/stores/rate', { storeId, rating: value })
...
// 	return (
// 		<div className="grid" style={{ gap: 16, width: 560, margin: '0 auto' }}>
// 			<Card title="Browse Stores" actions={<Button className="btn" onClick={search}>Search</Button>}>
// 				<div className="row" style={{ marginBottom: 12 }}>
// 					<Input placeholder="Search by name" value={query.name} onChange={e => setQuery({ ...query, name: e.target.value })} />
// 					<Input placeholder="Search by address" value={query.address} onChange={e => setQuery({ ...query, address: e.target.value })} />
...
// 							<th style={{ textAlign: 'center' }}>My Rating</th>
...
// 								<td>{s.name}</td>
// 								<td>{s.address}</td>
// 								<td style={{ textAlign: 'center' }}>{Number(s.averageRating).toFixed(2)}</td>
// 								<td style={{ textAlign: 'center' }}>{myRatings[s.id] ?? '-'}</td>
// 								<td style={{ textAlign: 'center' }}>
// 									{user ? (
// 										<Select value={myRatings[s.id] || ''} onChange={e => rate(s.id, Number(e.target.value))}>
			...
			<Card>
								...
								<tr key={s.id}>
									...
									<td className="text-center">
										<div className="font-medium">
											{myRatings[s.id] ? (
												<span className="text-success">★ {myRatings[s.id]}</span>
											) : (
				...
				{stores.length === 0 && (
					<div className="text-center py-8">
						<p className="text-muted">No stores found. Try adjusting your search criteria.</p>
					</div>
				)}
			</Card>
		</div>
